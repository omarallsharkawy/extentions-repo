...
class HexaWatch :
    ...
    // Site JS uses the typo host "theemoviedb" (not "themoviedb"); NXDOMAIN on the old name.
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        val type = filters.filterIsInstance<HexaWatchFilters.TypeFilter>().firstOrNull()?.state?.let {
            if (it == 0) "movie" else "tv"
        } ?: "movie"
        val sortFilter = filters.filterIsInstance<HexaWatchFilters.SortFilter>().firstOrNull()
        ...
        val genres = filters.filterIsInstance<HexaWatchFilters.GenreFilter>().firstOrNull()
