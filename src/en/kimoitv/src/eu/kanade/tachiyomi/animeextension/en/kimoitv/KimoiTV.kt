...
class KimoiTV : ParsedAnimeHttpSource() {
    ...
    override fun popularAnimeFromElement(element: Element): SAnime = SAnime.create().apply {
        ...
        thumbnail_url = element.selectFirst("img.content-poster, img")?.attr("abs:src")
        title = element.selectFirst("div.content-title")?.text()
            ?: link.selectFirst("img")?.attr("alt")?.takeIf { it.isNotBlank() }
            ?: link.attr("title").ifBlank { link.attr("href").substringAfterLast("/").substringBefore(".html") }
    }

    override fun popularAnimeNextPageSelector(): String = "ul.pagination > li.page-item > a.page-link:contains(Next)"
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        val firstSelected = filterList.firstOrNull {
            it is UriPartFilter && it.state != 0 && it !is SortFilter
        } as? UriPartFilter
    ...
    override fun animeDetailsParse(document: Document): SAnime = SAnime.create().apply {
        thumbnail_url = document.selectFirst("div#pilled img:not(.image)")?.attr("abs:src")
            ?: document.selectFirst("img.content-poster, div#pilled img")?.attr("abs:src")
        description = document.select("div#description > p, div#description p.card-text")
            .joinToString("\n\n") { it.text() }
        genre = document.select("div.chip a, div.chip .chip-label, div.chip")
            .map { it.text().trim() }
            .filter { it.isNotBlank() }
            .distinct()
            .joinToString(", ")
        title = document.selectFirst("div.content-title, h1")?.text()
    ...
    override fun episodeListParse(response: Response): List<SEpisode> {
        ...
        val seasonLinks = document.select(seasonListSelector())
        if (seasonLinks.isNotEmpty()) {
            seasonLinks.forEach { season ->
                var nextPageUrl: String? = season.attr("abs:href")
                val seasonName = season.selectFirst("span.font-weight-bold")?.text()
                    ?: season.ownText().ifBlank { season.text() }
                var counter = 1
                ...
                while (nextPageUrl != null) {
                    ...
                    doc.select(episodeListSelector()).forEach { ep ->
                        val epName = ep.ownText().trim().ifBlank { ep.text().trim() }
                        episodeList.add(
                            SEpisode.create().apply {
                                name = "$seasonName - $epName"
                                setUrlWithoutDomain(ep.attr("abs:href"))
                                episode_number = epName.substringAfterLast("E")
                                    .substringBefore(" ")
                                    .toFloatOrNull()
                                    ?: counter.toFloat()
            ...
            // Movies / single-season pages list episodes directly
            document.select(episodeListSelector()).forEachIndexed { index, ep ->
                val epName = ep.ownText().trim().ifBlank { ep.text().trim() }
                episodeList.add(
                    SEpisode.create().apply {
                        name = epName
    ...
    private fun seasonListSelector() = "a.downloads-link[href], a.list-group-item[href]"
    ...
    private val extractor by lazy { KimoiTVExtractor(client, headers) }

    // The stream RPC (POST /streamvpaid.php) is guarded by a Cloudflare Turnstile token,
    // so solving it requires a WebView; hence the suspend override instead of videoListParse.
    override suspend fun getVideoList(episode: SEpisode): List<Video> = extractor.videosFromUrl(baseUrl + episode.url)
