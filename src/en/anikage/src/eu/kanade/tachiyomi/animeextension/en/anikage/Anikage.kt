...
class Anikage :
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        ...
        return providers.toList().parallelCatchingFlatMap { (type, provider) ->
            ...
            val tracks = episodeData.subtitles.mapNotNull { sub ->
                val file = sub.file ?: return@mapNotNull null
                // Subtitle files may be prox-encoded like sources.
                val url = if (file.startsWith("http")) {
                    file
                } else {
                    "https://prox.anikage.cc/stream/$file"
                }
                Track(url, sub.label ?: "Subtitle")
            }

            val fromSources = episodeData.sources.parallelCatchingFlatMap { source ->
                val qualityLabel = source.quality ?: "auto"
                val videoUrl = source.episodeSourceUrl()
                if (source.isM3U8 == true || source.isM3U8 == null) {
                    playlistUtils.extractFromHls(
                        playlistUrl = videoUrl,
                        masterHeaders = proxyHeaders,
                        videoHeaders = proxyHeaders,
                        videoNameGen = { "$type - $provider - $qualityLabel - $it" },
                        subtitleList = tracks,
                    )
                } else {
                    Video(
                        url = videoUrl,
                        quality = "$type - $provider - $qualityLabel",
                        videoUrl = videoUrl,
            ...
            // Prefer native sources; fall back to direct embed m3u8 when present (prox can 403).
            fromSources.ifEmpty {
                episodeData.embeds.orEmpty().mapNotNull { embed ->
                    val url = embed.url
                    if (!url.contains(".m3u8", ignoreCase = true) && !url.contains("playlist", ignoreCase = true)) {
                        return@mapNotNull null
                    }
                    Video(
                        url = url,
                        quality = "$type - $provider - ${embed.server ?: embed.type ?: "embed"}",
                        videoUrl = url,
                        subtitleTracks = tracks,
                        headers = proxyHeaders,
                    )
    ...
    private fun parseAnime(response: Response): AnimesPage {
        ...
        val perPage = response.request.url.queryParameter("per_page")?.toIntOrNull() ?: 25
        val items = jsonData.items()
        val hasNextPage = jsonData.nextPage(page, perPage)

        val animes = items.map {
            val id = it.slug
            val titleFormat = preferences.titleStyle
            val titleName = if (titleFormat == "english") {
                it.title.english ?: it.title.romaji ?: it.title.userPreferred ?: id
            } else {
                it.title.romaji ?: it.title.english ?: it.title.userPreferred ?: id
            }
    ...
    companion object {
        ...
        // Site replaced advanced-search with browse (same query params).
        ...
        private const val PREF_SUB_DEFAULT = "neko"

        private const val PREF_DUB_SOURCE = "preferred_dub_source"
        private const val PREF_DUB_DEFAULT = "neko"
