...
class Mapple :
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        val sortFilter = filters.filterIsInstance<MappleFilters.SortFilter>().firstOrNull()
        ...
        val genres = filters.filterIsInstance<MappleFilters.GenreFilter>().firstOrNull()
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        ...
        // Site switched from Next.js server-actions + enc-dec.app/enc-mapple to
        // /api/encrypt → /api/stream-encrypted. Request token is embedded in the watch page.
        val tvSlug = if (episodeData.type == "tv") {
            "${episodeData.season}-${episodeData.episode}"
        } else {
            ""
        }
        val watchPath = if (episodeData.type == "tv" && tvSlug.isNotEmpty()) {
            "/watch/tv/${episodeData.tmdbId}-$tvSlug"
        } else {
            "/watch/${episodeData.type}/${episodeData.tmdbId}"
        }

        val requestToken = try {
            val watchHtml = client.newCall(GET(mappleApi + watchPath, headers))
                .awaitSuccess()
                .use { it.body.string() }
            REQUEST_TOKEN_REGEX.find(watchHtml)?.groupValues?.get(1)
        } catch (_: Exception) {
            null
        }
        ...
        val encryptHeaders = headers.newBuilder()
        ...
        val videoList = serversToQuery.parallelCatchingFlatMap { hoster ->
            val encryptBody = EncryptRequestDto(
                data = EncryptDataDto(
            ...
            val encryptResponse = client.newCall(
                POST(
                    "$mappleApi/api/encrypt",
                    encryptHeaders,
                    json.encodeToString(encryptBody).toJsonRequestBody(),
                ),
            ).awaitSuccess().use { it.parseAs<EncryptResponseDto>() }

            val encryptedPath = encryptResponse.url
                ?: return@parallelCatchingFlatMap emptyList()

            val streamApiUrl = buildString {
                append(if (encryptedPath.startsWith("http")) encryptedPath else mappleApi + encryptedPath)
                if (!requestToken.isNullOrBlank()) {
                    append(if (contains("?")) "&" else "?")
                    append("requestToken=")
                    append(java.net.URLEncoder.encode(requestToken, Charsets.UTF_8.name()))
                }
            }
            ...
            val videoResponse = client.newCall(GET(streamApiUrl, streamHeaders))
                .awaitSuccess()
                .use { it.parseAs<VideoResponseDto>() }
        ...
        if (videoList.isEmpty()) {
            throw Exception("Failed to fetch video streams")
        }
    ...
    companion object {
        ...
        private val REQUEST_TOKEN_REGEX = Regex("""window\.__REQUEST_TOKEN__\s*=\s*"([^"]+)""")

        private const val TMDB_API_KEY = BuildConfig.TMDB_API

        private const val PREF_DOMAIN_KEY = "pref_domain"
        private const val PREF_DOMAIN_DEFAULT = "https://mapple.rip"
        private val DOMAIN_ENTRIES = arrayOf("mapple.rip", "mapple.club")
        private val DOMAIN_VALUES = arrayOf("https://mapple.rip", "https://mapple.club")
