...
class AdultHtml5Player(
    ...
    private fun extractEncodedId(pageHtml: String, pageUrl: String): String? {
        ENCODED_ID_REGEX.find(pageHtml)?.groupValues?.get(1)
            ...
            ?.let { return it }
        DATA_VIDEO_ENCODED_ID_REGEX.find(pageHtml)?.groupValues?.get(1)
            ?.takeIf { it.isNotBlank() }
            ?.let { return it }
    ...
    private fun extractCdnId(pageHtml: String): String = CDN_ID_REGEX.find(pageHtml)?.groupValues?.get(1)
        ...
        ?: DATA_VIDEO_CDN_REGEX.find(pageHtml)?.groupValues?.get(1)
        ?: DEFAULT_CDN_ID
    ...
    companion object {
        ...
        /** Listing redesign embeds player ids in `data-video='{"encodedId":"..."}'`. */
        private val DATA_VIDEO_ENCODED_ID_REGEX = Regex(
            """["']encodedId["']\s*:\s*["']([^"']+)["']""",
        )
        private val DATA_VIDEO_CDN_REGEX = Regex(
            """["']picIdCdn["']\s*:\s*(\d+)""",
        )
