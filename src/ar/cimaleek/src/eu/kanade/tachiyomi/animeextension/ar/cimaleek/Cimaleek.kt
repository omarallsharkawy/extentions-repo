...
class Cimaleek :
    ...
    // r101.cimalek.buzz / cimalek.buzz redirect here
    override val baseUrl = "https://web.cimale3k.space"
    ...
    private var recentSlug: String? = null
    ...
    override fun latestUpdatesRequest(page: Int): Request {
        val slug = resolveRecentSlug()
        val url = if (page <= 1) "$baseUrl$slug/" else "$baseUrl$slug/page/$page/"
        return GET(url, headers)
    }

    private fun resolveRecentSlug(): String {
        recentSlug?.let { return it }
        val slug = try {
            val href = client.newCall(GET("$baseUrl/", headers)).execute().asJsoup()
                .selectFirst("a[href*=/recent-]")!!.attr("href")
            val path = if (href.startsWith("http")) href.toHttpUrl().encodedPath else href
