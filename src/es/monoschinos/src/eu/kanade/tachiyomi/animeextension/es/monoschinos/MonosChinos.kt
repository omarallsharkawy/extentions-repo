...
class MonosChinos :
    ...
    override fun popularAnimeParse(response: Response): AnimesPage {
        ...
        val elements = document.select("a.card-wrap[href*='/anime/'], li.ficha_efecto a")
        val nextPage = document.selectFirst(
            "a[href*='p=']:containsOwn(»), .pagination a:has(span:containsOwn(»)), a[href*='?p='][rel=next]",
        ) != null || run {
            val current = response.request.url.queryParameter("p")?.toIntOrNull() ?: 1
            document.select("a[href*='p=']").any { link ->
                link.attr("href").substringAfter("p=").substringBefore("&").toIntOrNull()?.let { it > current } == true
            }
        }
        val animeList = elements.mapNotNull { element ->
            SAnime.create().apply {
                title = element.selectFirst("h3.card-title, h3")?.text()
    ...
    override fun latestUpdatesParse(response: Response): AnimesPage {
        val document = response.asJsoup()
        // Redesign: homepage episode cards use a.card-wrap pointing to /ver/
        val episodeItems = document.select("a.card-wrap[href*='/ver/']")
        val animeList = if (episodeItems.isNotEmpty()) {
            episodeItems.mapNotNull { episodeLink ->
                ...
                val animeTitle = episodeLink.selectFirst("h3.card-title, h3")?.text()
                    ?: episodeLink.selectFirst("img")?.attr("alt")
                    ?: return@mapNotNull null
                SAnime.create().apply {
                    title = animeTitle
                        .replace(Regex("""\s*cap[ií]tulo\s*\d+.*$""", RegexOption.IGNORE_CASE), "")
                        .trim()
                    setUrlWithoutDomain(animeUrl)
                    thumbnail_url = episodeLink.selectFirst("img")?.getImageUrl()
                        ?: episodeLink.selectFirst("img")?.attr("abs:src")
                }
            }
        } else {
            // Legacy layout
            document.select("ul.row.row-cols-xl-4.row-cols-lg-4.row-cols-md-3.row-cols-2 > li.col.mb-4")
                .mapNotNull { item ->
    ...
    override fun animeDetailsParse(response: Response): SAnime {
        ...
        return SAnime.create().apply {
            title = document.selectFirst("h1.font-extrabold, h1.fs-2.text-capitalize.text-light, h1")?.text() ?: ""
            description = document.selectFirst("#tab-info p, #profile-tab-pane .mb-3 p")?.text()
            genre = document.select("a[href*='/genero/'], #profile-tab-pane .badge.bg-secondary")
                .joinToString { it.text() }
            thumbnail_url = document.selectFirst("img.lazy.absolute, .d-none.d-sm-flex img.lazy, img.lazy")?.getImageUrl()
            status = run {
                val pageText = document.text()
                when {
                    pageText.contains("En emisión", ignoreCase = true) ||
                        pageText.contains("Estreno", ignoreCase = true) -> SAnime.ONGOING
                    pageText.contains("Finalizado", ignoreCase = true) -> SAnime.COMPLETED
                    else -> {
                        val estadoElement = document.selectFirst(
                            ".col:has(.text-muted:contains(Estado)) div.ms-2 div:last-child",
                        )
    ...
    override fun episodeListParse(response: Response): List<SEpisode> {
        ...
        while (hasMore && currentPage <= maxPages) {
            ...
            // Site accepts GET for caplist; POST often trips Cloudflare without browser cookies.
            val request = Request.Builder()
                ...
                .apply {
                    if (csrfToken.isNotBlank()) header("X-CSRF-TOKEN", csrfToken)
                }
                .build()
            ...
            val json = try {
                ...
                // Fallback to legacy POST with token
                try {
                    val formBody = FormBody.Builder().add("_token", csrfToken).build()
                    client.newCall(
                        Request.Builder()
                            .url(paginatedUrl)
                            .post(formBody)
