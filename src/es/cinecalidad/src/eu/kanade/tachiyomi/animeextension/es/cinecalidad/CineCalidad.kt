...
class CineCalidad :
    ...
    override fun popularAnimeSelector(): String = "article.item[data-cf]"

    override fun popularAnimeRequest(page: Int): Request = GET("$baseUrl/page/$page")

    override fun popularAnimeFromElement(element: Element): SAnime = SAnime.create().apply {
        val img = element.selectFirst("img")
        title = img?.attr("alt").orEmpty().ifBlank { element.selectFirst("a")?.attr("title").orEmpty() }
        thumbnail_url = img?.attr("data-src")?.ifBlank { null } ?: img?.attr("src")
