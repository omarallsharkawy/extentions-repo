...
open class Serieskao :
    ...
    override fun popularAnimeSelector(): String = "a.card__link"

    override fun popularAnimeRequest(page: Int): Request = GET("$baseUrl/series?page=$page")

    override fun popularAnimeFromElement(element: Element): SAnime = SAnime.create().apply {
        setUrlWithoutDomain(element.attr("abs:href"))
        val rawTitle = element.selectFirst(".card__title, h2")?.text().orEmpty()
        ...
        thumbnail_url = thumb.replace("/w154/", "/w500/").replace("/w300/", "/w500/")
    }

    override fun popularAnimeNextPageSelector(): String = "a.pagination__btn[aria-label=Siguiente], a.pagination__btn:contains(›), .pagination__list a.pagination__btn:not(.pagination__btn--active)"
    ...
    override fun episodeListParse(response: Response): List<SEpisode> {
        ...
        val seasonTabs = doc.select("#season-tabs li a[data-tab], #temporadas_select option")
        val numberRegex = Regex("\\d+")

        if (seasonTabs.isEmpty()) {
            doc.select(".episodes-list a.episode-item").forEachIndexed { index, element ->
                val episodeNumber = element.selectFirst(".episode-item__number, .episode-number")?.text()
                    ?.let { numberRegex.find(it)?.value } ?: (index + 1).toString()
                val episodeTitle = element.selectFirst(".episode-item__title, .episode-title")?.text()
            ...
            // Current redesign loads one season pane; also keep legacy tab panes.
            val seasonPanes = doc.select(".episodes-list[id^=season-], .episodes-list")
            if (seasonPanes.isNotEmpty() && doc.select("#season-tabs li a[data-tab]").isEmpty()) {
                seasonPanes.forEach { pane ->
                    val seasonNumber = numberRegex.find(pane.id())?.value ?: "1"
                    pane.select("a.episode-item").forEach { element ->
                        val episodeNumber = element.selectFirst(".episode-item__number, .episode-number")?.text()
                ...
                seasonTabs.forEachIndexed { index, tab ->
                    val seasonId = tab.attr("data-tab").ifBlank { tab.attr("value") }
                    ...
                    seasonPane.select(".episodes-list a.episode-item, a.episode-item").forEach { element ->
                        val episodeNumber = element.selectFirst(".episode-item__number, .episode-number")?.text()
                            ?.let { numberRegex.find(it)?.value } ?: "0"
                        val episodeTitle = element.selectFirst(".episode-item__title, .episode-title")?.text()
                            ?.ifBlank { "Episodio $episodeNumber" } ?: "Episodio $episodeNumber"
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        // New player: buttons with /vidurl/... embeds that contain encrypted dataLink
        val embedPaths = document.select("button.server-btn[data-url], .player-box__servers button[data-url]")
            .mapNotNull { it.attr("data-url").takeIf { url -> url.isNotBlank() } }
            .distinct()

        if (embedPaths.isNotEmpty()) {
            return embedPaths.parallelCatchingFlatMapBlocking { path ->
                val videoUrl = if (path.startsWith("http")) path else baseUrl + path
                ...
                val bodyDoc = Jsoup.parse(body, videoUrl)
        ...
        // Legacy player: var videoSources = [...]
    ...
    private fun extractNewExtractorLinks(doc: Document, htmlContent: String): List<Pair<String, String>>? {
        ...
        val powKey = solvePowKey(htmlContent)
    ...
    private fun decryptEmbedLink(rawLink: String?, powKey: ByteArray? = null): String? {
        ...
        // New embed69 PoW key (AES-CBC, IV prepended to ciphertext)
        if (powKey != null) {
            runCatching {
                val encryptedData = Base64.decode(link, Base64.DEFAULT)
                if (encryptedData.size > 16) {
                    ...
                    CryptoAES.decrypt(cipherB64, powKey, iv).takeIf { it.startsWith("http", true) }
                } else {
                    null
                }
    ...
    private fun solvePowKey(html: String): ByteArray? {
    ...
    override fun animeDetailsParse(document: Document): SAnime = SAnime.create().apply {
        title = document.selectFirst("h1.detail-hero__title, h1.m-b-5, h1")?.text()?.ifBlank { "Sin título" }
            ?: "Sin título"
        thumbnail_url = document.selectFirst(
            "figure.detail-hero__poster img, div.card-body div.row div.col-sm-3 img.img-fluid, .detail-hero__poster img",
        )?.attr("src")?.replace("/w154/", "/w500/")?.replace("/w300/", "/w500/")
        description = document.selectFirst(".detail-hero__desc, div.col-sm-4 div.text-large")?.text()
        genre = document.select("a.detail-hero__genre, div.p-v-20.p-h-15.text-center a span")
