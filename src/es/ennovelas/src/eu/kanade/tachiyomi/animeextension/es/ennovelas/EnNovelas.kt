...
class EnNovelas :
    ...
    override fun popularAnimeFromElement(element: Element): SAnime {
        ...
        anime.setUrlWithoutDomain(link?.attr("href").orEmpty())
        anime.title = element.selectFirst("a .title")?.text()
            ?.ifBlank { null }
            ?: link?.attr("title")
            ?: link?.text().orEmpty()
        val dataImg = element.selectFirst("[data-img]")?.attr("data-img").orEmpty()
        anime.thumbnail_url = when {
            dataImg.contains("url(") -> dataImg.substringAfter("url(").substringBefore(")").trim('"', '\'')
            dataImg.isNotBlank() -> dataImg
    ...
    override fun popularAnimeNextPageSelector(): String = "link[rel=next], .pagination .current ~ a, .navigation a.next, a[href*='/series/page/']"
