...
class FlixLatam :
    ...
    override fun popularAnimeRequest(page: Int) = GET("$baseUrl/peliculas/populares?page=$page")

    override fun popularAnimeSelector() = latestUpdatesSelector()

    override fun popularAnimeNextPageSelector() = latestUpdatesNextPageSelector()

    override fun latestUpdatesRequest(page: Int) = GET("$baseUrl/peliculas?page=$page")

    override fun latestUpdatesSelector() = "div.items article.item > div.poster"

    override fun latestUpdatesNextPageSelector() = "div.pagination a:contains(Siguiente), div.pagination span.current ~ a"

    override fun searchAnimeSelector() = "article.item > div.poster, div.result-item div.image a"

    override fun searchAnimeFromElement(element: Element): SAnime {
        // New listing cards use poster wrappers; legacy search used direct anchors.
        return if (element.tagName() == "a" && element.hasAttr("href")) {
            super.searchAnimeFromElement(element)
        } else {
    ...
    override fun popularAnimeFromElement(element: Element): SAnime = SAnime.create().apply {
        val article = generateSequence(element) { it.parent() }
            .firstOrNull { it.tagName() == "article" }
            ?: element
        val img = element.selectFirst("img") ?: article.selectFirst("img")
        val url = element.selectFirst("a[href]")?.attr("href")
            ?: article.selectFirst("a[href]")?.attr("href")
            ?: element.attr("href")
        setUrlWithoutDomain(url)
        title = article.selectFirst("div.data h3 a, h3 a")?.text()
            ?.ifBlank { null }
            ?: img?.attr("alt")?.removePrefix("Ver ")?.removeSuffix(" online")
            ?: ""
        thumbnail_url = img?.attr("src")?.ifBlank { null } ?: img?.attr("data-src")
    }
    ...
    override fun videoListSelector() = "div.play iframe[src], ul#playeroptionsul li, ul#playerOptions li"
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        // New site layout: direct /vidurl/ iframe (embed69)
        val iframeUrls = document.select("div.play iframe[src], div.pframe iframe[src], .dooplay_player iframe[src]")
            .mapNotNull { el ->
            ...
            .map { src ->
                when {
                    ...
                    else -> baseUrl.trimEnd('/') + if (src.startsWith("/")) src else "/$src"
                }
            }
            .distinct()
        ...
        // Legacy dooplay ajax player
        val players = document.select("ul#playeroptionsul li")
        return players.parallelCatchingFlatMapBlocking { player ->
            val url = getPlayerUrl(player)
                ?: return@parallelCatchingFlatMapBlocking emptyList()
            if (url.contains("embed69") || url.contains("vidurl")) {
                val htmlContent = client.newCall(GET(url, embedHeaders)).awaitSuccess().bodyString()
                if (htmlContent.isBlank()) return@parallelCatchingFlatMapBlocking emptyList()
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        return when {
            ...
            params.genre.isNotBlank() -> {
                val genrePath = if (params.genre in listOf("ratings", "tendencias", "pelicula", "peliculas", "series", "animes")) {
                    params.genre
                } else {
                    "generos/${params.genre}"
                }
                GET("$baseUrl/$genrePath?page=$page")
            }
